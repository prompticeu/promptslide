        ...
        <div className="grid w-full grid-cols-3 gap-10">
          {metrics.map((metric, index) => (
            <Animated key={metric.label} step={index + 1} animation="slide-up">
              <div>
                <div className="text-6xl font-bold tracking-tight text-primary">{metric.value}</div>
                <div
                  className={`mt-4 h-1 rounded-full bg-gradient-to-r from-primary to-primary/30 ${metric.barWidth}`}
                />
                <div className="mt-3 text-lg font-semibold text-foreground">{metric.label}</div>
